...
export let des = {
  ...
  encryptStr: function(data: string, key: string): string {
    let buffer = Buffer.from(data, 'utf8');
    return this.encrypt(buffer, key).toString('base64');
  },
  decryptStr: function(data: string, key: string) {
    let buffer = Buffer.from(data, 'base64');
    return this.decrypt(buffer, key).toString('utf8');
  },
};
